...
Use tabs on a single page when users switch between related sections, such as profile details, order history, or app settings, without leaving the screen.
...
In WaveMaker, the Panel component is a versatile UI container that can include a title, optional subheading, icon, content area, and optional footer with actions such as buttons or forms. Panels are commonly used to present grouped information in cards, summaries, and form sections on a page.

Set **`collapsible`** to allow users to expand or collapse the panel body from the header. When **`collapsible`** is enabled, tapping the header toggles **`expanded`**. **`expanded`** defaults to `true`, so content is visible until the user collapses the panel. The header shows expand and collapse icons (override with **`expandediconclass`** and **`collapsediconclass`** if needed). Wire **`on-expand`** and **`on-collapse`** in the page script when you need to run logic after the panel opens or closes. A panel can also load a **partial** as its content, in addition to child widgets placed in markup.

**Markup:**

```html
<wm-panel subheading="subheading" iconclass="wi wi-account-circle" title="Title" collapsible="true" expanded="true" name="panel1" class="panel panel-default" variant="default:default">
    <wm-panel-footer name="panel_footer1">
        <wm-label padding="unset 0.5em" class="text-muted p" caption="Additional Info" name="label1" variant="default:p"></wm-label>
    </wm-panel-footer>
</wm-panel>
```

- `<wm-panel>` acts as a structured container that groups related content into a single visual block.
- Use **`collapsible`** and **`expanded`** to control whether the body can be toggled and whether it starts open.
...
In WaveMaker, the Tile widget allows you to add child widgets such as images, text, icons, and labels. While it functions similarly to a Container, a Tile comes with **default styling** that makes it more suitable for dashboard-style layouts. This built-in styling helps create a consistent look and feel without additional configuration.
