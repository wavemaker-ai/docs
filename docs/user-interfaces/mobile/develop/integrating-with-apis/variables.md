...
- **Data Binding**: Bind variable data directly to widget properties (for example, binding a List `dataset` to a service variable).
- **Event Binding**: Trigger variables on UI events such as `onTap`, `onLoad`, or `onChange`.
...
For bind dialog options, expression formatters, and expression examples, see [Bind expressions](/docs/user-interfaces/mobile/develop/integrating-with-apis/bind-expressions).
