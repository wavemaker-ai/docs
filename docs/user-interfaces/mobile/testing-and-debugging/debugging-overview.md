...
### 4. Source maps and breakpoints

Expo and Metro enable **source maps in development by default** when you run `npx expo start`. You usually do not need to change `metro.config.js` in an exported WaveMaker project.

If breakpoints in [React Native DevTools](./community-debugging-tools/react-native-devtools) Sources panel do not bind:

- Reload the app after setting breakpoints (`r` in the Expo terminal, or **Reload** in the developer menu)
- Use a `debugger;` statement in page script as a fallback
- Clear the Metro cache: `npx expo start --clear`
- Confirm you are debugging a **development** build, not a release APK/IPA

Only customize `metro.config.js` if your team already maintains that file for other reasons. Follow [Expo Metro documentation](https://docs.expo.dev/guides/customizing-metro/) rather than adding a standalone `sourceMap: true` root option, which does not match the default Expo config shape.
