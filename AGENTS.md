...
- MDX components registered in `src/theme/MDXComponents/index.js` (e.g., `AcademyCard`, `VideoCard`, `Pill`, `PillGroup`, `Accordian`, `TabsWrapper`, `TabItem`, `ReleaseNotesTabs`) are globally available in any `.mdx` file. Do not import them — it causes redeclaration errors.
- `TabsWrapper`/`TabItem`/`Accordian` are generic, content-agnostic layout primitives — usable anywhere, with no knowledge of each other. `ReleaseNotesTabs` is a release-notes-specific wrapper built on top of them (used only in `docs/release-notes/`): it auto-computes each tab's item-count badge and auto-expands the first non-empty accordian. Use `ReleaseNotesTabs` as the outer wrapper for release notes instead of `TabsWrapper`; use `TabsWrapper` directly for any other generic tabbed content.
